...
public class ProductsAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        vh.iv_pop_products_img.setTag(products.getProducts_icon());
        utils.showImageFromAsyncTask(vh.iv_pop_products_img, products.getProducts_icon(), context);
            ...
            public void onClick(View v) {//liveid op products_id  参数  post(eventUrl + "applogin", param);
                ...
                if (!pList.get(index).isClick()) {
                    new Thread() {
                        ...
                        public void run() {
                            super.run();
                            String post = Utils.post(url, params);
                            Log.i("Adatper", post);
                        }
                    }.start();
                    vh.btn_pop_products_isRecommend.setText("已推荐");
